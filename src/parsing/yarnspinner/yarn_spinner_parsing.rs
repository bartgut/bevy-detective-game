...
pub fn load_from_file(file_name: &str) -> Vec<Node> {
    ...
    let dialog = fs::read_to_string(file_name)
    ...
    for section in parsed.into_iter() {
        let mut node = Node { title: "".to_string(), lines: vec![] };
                                    ...
                                    Rule::set_line => {
                                        ...
                                        for set_line_field in content.into_inner() {
                                            match set_line_field.as_rule() {
                                                Rule::variable_name => {
                                                    variable_name = set_line_field.as_str().to_string();
                                                },
                                                Rule::boolean_value => {
                                                    value = set_line_field.as_str().to_string().parse::<bool>().unwrap();
                                                },
                                                _ => unreachable!()
                                            }
                                        }
                                    ...
                                    Rule::dialog_line => {
                                        ...
                                        for dialog_line_field in content.into_inner() {
                                            match dialog_line_field.as_rule() {
                                                Rule::speaker => {
                                                    speaker = dialog_line_field.as_str().to_string();
                                                },
                                                Rule::dialog => {
                                        ...
                                        node.lines.push(LineType::DialogLine { speaker: speaker, text: text, condition: condition });
                                    },
                                    Rule::option_lines => {
                                        let mut optionPossiblitites: Vec<OptionPossibility> = vec![];
                                        let mut speaker = "".to_string();
                                        ...
                                        node.lines.push(LineType::OptionLine { speaker: speaker, possibilites: optionPossiblitites });
                                    },
                                    Rule::jump_line => {
                                        for jump_line_field in content.into_inner() {
                                            match jump_line_field.as_rule() {
                                                Rule::title => {
                                                    node.lines.push(LineType::JumpLine { node_title: jump_line_field.as_str().to_string() });
                                                },
                                                _ => unreachable!()
                                            }
                                        }
